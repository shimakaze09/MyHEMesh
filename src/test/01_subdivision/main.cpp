...
class E : public TEdge<TraitsVEP> {
 ...
 public:
  const string Name() const {
    return "[" + pre + "]" + HalfEdge()->Origin()->name + "-" +
           HalfEdge()->End()->name;
  }
...
int main() {
  ...
  mesh->AddPolygon(
      std::array{e34->HalfEdge(), e45->HalfEdge(), e53->HalfEdge()}, "P345");
  mesh->AddPolygon(
      std::array{v4->HalfEdgeTo(v3), v3->HalfEdgeTo(v1), v1->HalfEdgeTo(v4)},
      "P431");
  mesh->AddPolygon(
      std::array{v5->HalfEdgeTo(v4), v4->HalfEdgeTo(v2), v2->HalfEdgeTo(v5)},
      "P542");
  mesh->AddPolygon(
      std::array{v3->HalfEdgeTo(v5), v5->HalfEdgeTo(v0), v0->HalfEdgeTo(v3)},
      "P350");
  Print(mesh);
