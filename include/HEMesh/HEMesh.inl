...
const typename HEMesh<V>::ptrE HEMesh<V>::AddEdge(ptr<V> v0, ptr<V> v1,
                                                  Args&&... args) {
  if (v0 == v1) {
    printf(
...
template <typename V>
template <typename... Args>
const typename HEMesh<V>::ptrP HEMesh<V>::AddPolygon(
    const std::vector<ptr<HE>> heLoop, Args&&... args) {
  if (heLoop.size() == 0) {
    printf(
...
template <typename V>
const std::vector<std::vector<typename HEMesh<V>::ptrHE>>
HEMesh<V>::Boundaries() {
...
bool HEMesh<V>::IsValid() const {
  ...
  for (auto e : edges) {
    if (!e->HalfEdge() || e->HalfEdge()->Edge() != e ||
        e->HalfEdge()->Pair()->Edge() != e)
      return false;
  }
  for (auto p : polygons) {
    if (!p->HalfEdge())
...
template <typename V>
template <typename... Args>
const typename HEMesh<V>::ptrV HEMesh<V>::AddEdgeVertex(ptr<E> e,
                                                        Args&&... args) {
  // prepare
...
template <typename V>
template <typename... Args>
const typename HEMesh<V>::ptrE HEMesh<V>::ConnectVertex(ptr<HE> he0,
                                                        ptr<HE> he1,
                                                        Args&&... args) {
  auto p = he0->Polygon();
  if (p != he1->Polygon()) {
    printf(
        "ERROR::HEMesh::ConnectVertex:\n"
        "\t"
...
template <typename V>
template <typename... Args>
const typename HEMesh<V>::ptrV HEMesh<V>::CollapseEdge(ptr<E> e,
                                                       Args&&... args) {
  auto he01 = e->HalfEdge();
  auto he10 = he01->Pair();
