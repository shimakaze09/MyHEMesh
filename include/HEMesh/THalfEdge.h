...
class THalfEdge {
 ...
 public:
  ...
  const HE* const RotateNext() const {
    return const_cast<HE*>(this)->RotateNext();
  }

  const HE* const RotatePre() const {
  ...
  static const std::vector<const HE*> NextBetween(const HE* begin,
                                                  const HE* end) {
    return Const(NextBetween(const_cast<HE*>(begin), const_cast<HE*>(end)));
  }

  // [this, end), NextBetween(this, end);
  const std::vector<HE*> NextTo(HE* end) { return NextBetween(this, end); }

  const std::vector<const HE*> NextTo(const HE* end) const {
  ...
  static const std::vector<const HE*> RotateNextBetween(const HE* begin,
                                                        const HE* end) {
    return Const(
        RotateNextBetween(const_cast<HE*>(begin), const_cast<HE*>(end)));
  }

  // [this, end), RotateNextBetween(this, end);
  const std::vector<HE*> RotateNextTo(HE* end) {
  ...
  const std::vector<const HE*> RotateNextLoop() const {
