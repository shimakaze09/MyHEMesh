...
class TLoopIteratorBase {
 public:
  ...
  constexpr TLoopIteratorBase(Data data, bool is_begin) noexcept
      : data{data}, is_begin{is_begin} {}
  ...
  Impl operator++(int) noexcept {
    Impl rst = *static_cast<Impl*>(this);
    this->operator++();
    return rst;
  }
...
class TLoopViewBase : public std::ranges::view_base {
 public:
  ...
  Iterator begin() const noexcept {
    return begin_value ? Iterator{begin_value, true} : Iterator{};
  }
